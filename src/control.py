...
def run_and_wait(command, input="", fin=None):
    ...
    fout.seek(0)
    output = fout.read().decode("utf-8")
    ferr.seek(0)
    error = ferr.read().decode("utf-8")
    fin.close()
    fout.close()
    ferr.close()
...
class Mace4:
    ...
    def get_info_from_stderr(self, lines):
        ...
        if line:
            ...
            if m:
                return [
                    ("CPU Seconds", m.groups()[2]),
                    ("Domain Size", m.groups()[0]),
                    ("Models", m.groups()[1]),
                ]
            else:
                error_dialog("get_stderr_info, failed to match")
        return [("CPU Seconds", "?"), ("Domain Size", "?"), ("Models", "?")]
...
class Reformat_proof:
    def __init__(self, parent, proofs, num_proofs, saved_flag):
        ...
        self.dlg = dlg = wx.Dialog(
            parent, -1, title="Reformat Proof(s)", pos=pos_for_center((0, 0))
        )
...
class Run_program:
    ...
    def run(self):
        ...
        if not search_command or not success_command:
            ...
            self.fin.write(bytes(self.input, "utf-8"))
            self.fin.seek(0)
            ...
            self.output = self.fout.read().decode("utf-8")

            if self.exit_code == 0 or self.program.exists_solution(
                ...
                if rc == 0:
                    ...
                    self.solution = (
                        "There was an error extracting the %s."
...
class Program_panel(wx.Panel):
    def __init__(self, parent, program, options):
        ...
        if opt:
            ...
            self.time_ctrl = wx.SpinCtrl(
                self, id, min=-1, max=sys.maxsize, size=(75, -1)
            )
        ...
        time_sizer.Add(
            wx.StaticText(self, -1, "seconds."), 0, wx.ALL | wx.ALIGN_CENTER, 1
        )
        ...
        self.state_text = wx.StaticText(self, -1, "Ready", size=(width, -1))
    ...
    def on_info(self, evt):
        if self.job:
            ...
            self.info_panel = Mini_info(
                self, "Info on %s Search" % self.program.name, info
            )
            self.info_btn.Enable(False)
            if self.job.state != State.done:
                self.timer = wx.Timer(self, -1)
                self.Bind(wx.EVT_TIMER, self.update_info, id=self.timer.GetId())
    ...
    def job_finished(self):
        ...
        if self.job.state == State.error:
            ...
            if self.job.exit_code == 1:  # fatal error
                ...
                frame.Show(True)
                error_dialog(
                    "A Fatal Error occurred.  The %s output is shown."
                    % self.program.name
                )
...
class Control_panel(wx.Panel):
    def __init__(self, parent):
        ...
        self.prover9 = Program_panel(
            self, Prover9(), to_top(self).setup.p9_options.panels
        )
        self.mace4 = Program_panel(self, Mace4(), to_top(self).setup.m4_options)
...
class Isofilter_frame(wx.Frame):
    def __init__(self, parent, models, saved_solution):
        ...
        self.alg_ch = wx.Choice(
            self,
            self.alg_id,
            choices=["Occurrence Profiles", "Canonical Forms"],
        )
    ...
    def run(self):
        ...
        self.fin.write(bytes(self.models, "utf-8"))
        self.fin.seek(0)
        ...
        if Win32():
            ...
            self.process = subprocess.Popen(
                self.command, stdin=self.fin, stdout=self.fout, stderr=self.ferr
            )

        self.state = State.running
        self.exit_code = self.process.wait()  # Wait for process to finish!
        self.fout.seek(0)  # rewind stdout
        self.filtered_models = self.fout.read().decode("utf-8")
    ...
    if not prover9_command:
        ...
        if exit_str in ["Killed", "Crashed", "Interruppted"]:
            ...
            if re.search("%%ERROR", output):
                ...
                r = re.compile(
                    "(?<=%%START ERROR%%).*(?=%%END ERROR%%)", re.DOTALL
                )  # allow to cross lines
                m = r.search(output)
                if m:
