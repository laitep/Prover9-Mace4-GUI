...
class Input_panel(wx.Panel):
    def __init__(self, parent, title, auto_highlight):
        ...
        self.ed = wx.TextCtrl(
            self,  # Win32 neews TE_RICH2 for color
    ...
    def start_auto_highlight(self):
        if self.timer:
            ...
            self.Bind(wx.EVT_TIMER, self.check_highlight, id=self.timer.GetId())
        ...
        if exit == "Okay":
            ...
            if error:
                ...
                if start >= 0:
                    ...
                    self.ed.SetStyle(
                        start, end, wx.TextAttr("RED", wx.Colour(200, 200, 255))
                    )
...
class Language_panel(wx.Panel):
    def __init__(self, parent, options):
        ...
        self.input = Input_panel(
            self, "Language Options", to_top(self).auto_highlight()
        )
...
class Setup_tabs(wx.Notebook):
    def __init__(self, parent):
        ...
        link_options_by_names(
            self.p9_options.panels, self.m4_options, ["prolog_style_variables"]
        )

        # Additional Input Tab

        self.add = wx.SplitterWindow(self, -1)
        self.add_p9 = Input_panel(
            self.add,
            "Additional Input for Prover9",
            to_top(self).auto_highlight(),
        )
        self.add_m4 = Input_panel(
            self.add,
    ...
    def assemble_input(self):
        if Mac():
            # If the last thing the uswer did was to update a SpinCtrl (parm)
            # using text (instead of the arrows), the update will might not
            # really have occurred, because the focus is still on the SpinCtrl.
            # The following kludge seems to fix the problem by setting the
            # focus to something else. This problem occurs only on the Mac.
            top = to_top(self)
            top.control.SetFocusIgnoringChildren()
    ...
    def store_input(self, input):
        ...
        opt_x = set_options_either(
            opt, self.p9_options.panels, self.m4_options, handle_dep=handle_dep
        )
    ...
    def store_new_input(self, input, version):
        ...
        if version:
            if input.find("Saved by Prover9-Mace4") < 0:
                info_dialog(
                    "The file being opened was not created by"
                ...
                info_dialog(
                    "The file being opened was created with a"
