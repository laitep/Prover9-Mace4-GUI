        ...
        if start > 0:
            ...
            if end > 0:
                self.txt.SetStyle(
                    ...
                    wx.TextAttr("RED", wx.Colour(200, 200, 255)),
                )
        else:
...
class Mini_info(wx.MiniFrame):
    def __init__(self, parent, title, items):
        ...
        gsizer = wx.GridSizer(2)

        self.val_labels = []
        for name, val in items:
            name_lab = wx.StaticText(self, -1, name + ":")
            val_lab = wx.StaticText(
                self,
                -1,
                str(val),
                size=(75, -1),
...
class Busy_bar(wx.Gauge):
    ...
    def start(self):
        ...
        self.Bind(wx.EVT_TIMER, self.update_bar, id=self.timer.GetId())
